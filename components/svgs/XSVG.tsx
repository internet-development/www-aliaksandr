export default function XSVG(props) {
  return (
    <svg {...props} width={props.width ?? '24'} height={props.height ?? '24'} viewBox="0 0 300 301" fill={props.color ?? '#FFF6E5'} xmlns="http://www.w3.org/2000/svg">
      <path
        d="M178.57 127.15L290.27 0H263.81L166.78 110.38L89.34 0H0L117.13 166.93L0 300.25H26.46L128.86 183.66L210.66 300.25H300M36.01 19.54H76.66L263.79 281.67H223.13"
        fill={props.color ?? '#FFF6E5'}
      />
    </svg>
  );
}
